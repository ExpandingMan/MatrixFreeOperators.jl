...
- **Adaptive grids (AMR) — built end-to-end, including the packed-storage GPU
  ...
  - **Built (coarse–fine phase):** operators apply across refinement levels. The
    ...
    weights, whose Enzyme rules declare zero derivative. Measured, not assumed:
    `Σ V·(Lu)` on a refined forest telescopes to roundoff (test/forest_diffusion.jl),
    and it did not before the rewrite. This is the coefficient-transfer input #54's
    generalization needs. Quadratic exactness of every weight set is
