...
@testset "Enzyme custom rules (declared transposes, not the tape)" begin
    ...
    @testset "parameter gradients still tape (rules must not swallow them)" begin
        ...
        dκ = zero(κ)
        # Skipped on Julia 1.13 for now. The first autodiff below hits the same
        # EnzymeRuntimeActivityError as the κ test in autodiff.jl (allocate_output(::Gradient, …)
        # on 1.13 only). The Diffusion half already opts into runtime activity and still runs.
        # Re-enable once Enzyme handles it.
        if VERSION < v"1.13"
            Enzyme.autodiff(
                Enzyme.Reverse,
                er_kappa_loss,
